...
# 2. Use the Python executable from the virtual environment directly
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot'; & '$venvPython' backend\aggregator.py"

Write-Host "Starting Kafka Consumer (scores transactions)..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot'; & '$venvPython' backend\consumer_predict_confluent.py"

Write-Host "Starting Transaction Producer (sends live data)..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot'; & '$venvPython' backend\producer_live.py"

Write-Host "Starting FastAPI server (port 8000)..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot'; & '$venvPython' -m uvicorn backend.main:main --host 0.0.0.0 --port 8000 --reload"

Write-Host "Starting Live Monitor Dashboard (port 8501)..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot'; & '$venvPython' -m streamlit run frontend/dashboard.py --server.port 8501"

Write-Host "Starting Account Investigation Dashboard (port 8502)..." -ForegroundColor Yellow
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$PSScriptRoot'; & '$venvPython' -m streamlit run frontend/get.py --server.port 8502"
...
Write-Host ""
Write-Host "To stop everything, simply close all PowerShell windows."